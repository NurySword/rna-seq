...
points(fpkm[9463,] ~ jitter(as.numeric(pheno_data$type)), col=as.numeric(pheno_data$type)+1, pch=16)

# Create a plot of transcript structures observed in each replicate and color transcripts by expression level
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample TUMOR_Rep1'), sample=c('TUMOR_Rep1'))
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample TUMOR_Rep2'), sample=c('TUMOR_Rep2'))
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample TUMOR_Rep3'), sample=c('TUMOR_Rep3'))
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample NORMAL_Rep1'), sample=c('NORMAL_Rep1'))
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample NORMAL_Rep2'), sample=c('NORMAL_Rep2'))
plotTranscripts(ballgown::geneIDs(bg)[9463], bg, main=c('Gene in sample NORMAL_Rep3'), sample=c('NORMAL_Rep3'))
