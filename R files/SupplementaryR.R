...
working_dir = "/home/nury/workspace/rnaseq/de/ballgown/ref_only" 
setwd(working_dir)
...
gene_expression[1:3, c("FPKM.NORMAL_Rep1","FPKM.NORMAL_Rep2","FPKM.NORMAL_Rep3","FPKM.TUMOR_Rep1","FPKM.TUMOR_Rep2","FPKM.TUMOR_Rep3")]
...
i = row.names(gene_expression) == "ENSG00000233360"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once? 
#genes_of_interest = c("TST", "MMP11", "LGALS2", "ISX")
genes_of_interest = c("ENSG00000233360","ENSG00000271672","ENSG00000100234","ENSG00000100167")
...
min(gene_expression[,"FPKM.NORMAL_Rep1"])
max(gene_expression[,"FPKM.NORMAL_Rep1"])
...
data_columns=c(1:6)
short_names=c("NORMAL_1","NORMAL_2","NORMAL_3","TUMOR_1","TUMOR_2","TUMOR_3")
...
x = gene_expression[,"FPKM.NORMAL_Rep1"]
y = gene_expression[,"FPKM.NORMAL_Rep2"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (NORMAL, Replicate 1)", ylab="FPKM (NORMAL, Replicate 2)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (NORMAL, Replicate 1)", ylab="FPKM (NORMAL, Replicate 2)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.NORMAL_Rep1", "FPKM.TUMOR_Rep1", "NORMAL_1 vs TUMOR_1", "tomato2")
plotCor("FPKM.NORMAL_Rep2", "FPKM.TUMOR_Rep2", "NORMAL_2 vs TUMOR_2", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.NORMAL_Rep1", "FPKM.TUMOR_Rep1", "NORMAL_1 vs TUMOR_1", "tomato2")
plotCor2("FPKM.NORMAL_Rep2", "FPKM.TUMOR_Rep2", "NORMAL_2 vs TUMOR_2", "royalblue2")
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) NORMAL vs TUMOR", main="Distribution of differential expression values")
abline(v=-2, col="black", lwd=2, lty=2)
abline(v=2, col="black", lwd=2, lty=2)
legend("topleft", "Fold-change > 4", lwd=2, lty=2)

#### Plot #10 - Display the grand expression values from NORMAL and TUMOR and mark those that are significantly differentially expressed
gene_expression[,"NORMAL"]=apply(gene_expression[,c(1:3)], 1, mean)
gene_expression[,"TUMOR"]=apply(gene_expression[,c(4:6)], 1, mean)

x=log2(gene_expression[,"NORMAL"]+min_nonzero)
y=log2(gene_expression[,"TUMOR"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="NORMAL FPKM (log2)", ylab="TUMOR FPKM (log2)", main="NORMAL vs TUMOR FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
